...
@Component({
  standalone: true,
  imports: [TwitterShareComponent],
  template: `<twitter-share [shareRouterLink]="link()" [queryParams]="queryParams()" [size]="size()" [text]="text()"></twitter-share>`,
})
class HostComponent {
...
describe('TwitterShareComponent', () => {
  ...
  const rendered = () => fixture.nativeElement.querySelector('a.twitter-share-button');
  ...
  describe('loading the SDK', () => {
    it('requests the vendor SDK once a link is bound', async () => {
      ...
      expect(loadScript).toHaveBeenCalledWith('https://platform.twitter.com/widgets.js');
    });
  ...
  describe('rendering', () => {
    ...
    it('renders the static twitter href and count', async () => {
      await render(['/about']);
      expect(rendered()!.getAttribute('href')).toBe('https://twitter.com/share');
      expect(rendered()!.getAttribute('data-count')).toBe('none');
      expect(rendered()!.textContent).toBe('Tweet');
    });
  ...
  describe('the vendor global', () => {
    it('asks the SDK to re-parse the widget', async () => {
      (window as Record<string, unknown>)['twttr'] = { widgets: { load: vi.fn() } };

      await render(['/about']);

      expect((window as any)['twttr'].widgets.load).toHaveBeenCalled();
    });
