...
import { Directive, ElementRef, Input, Renderer2, inject, HostAttributeToken } from '@angular/core';
...
export class AdvancedRouterLinkDirective extends RouterLink {
   ...
   */
   ...
   set advRouterLink(commands: any[] | string | null | undefined) {
     ...
     // handling, from a private `_urlTree` computed that reads the
     // `routerLink` signal input -- not from the public `urlTree` getter,
     // which a subclass used to be able to override. Leaving `routerLink`
     // unset leaves `_urlTree()` null, which strips the href attribute
     // entirely and makes onClick a no-op.
